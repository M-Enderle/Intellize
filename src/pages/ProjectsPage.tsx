      ...
      <div className="max-w-screen-2xl mx-auto px-6 lg:px-12">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {PROJECTS.map((project, idx) => (
            ...
            >
              <Link to={`/projects/${project.id}`} className="group flex flex-col h-full bg-white rounded-3xl overflow-hidden border border-gray-200 hover:shadow-2xl hover:shadow-blue-900/10 transition-all duration-300">
                ...
                <div className="p-8 flex flex-col flex-grow bg-white/60 backdrop-blur-md">
                  ...
                  <div className="flex items-center justify-between pt-6 border-t border-gray-100/50 mt-auto">
                    <span className="inline-flex items-center text-blue-600 font-semibold text-sm group-hover:translate-x-2 transition-transform">
                      Projekt ansehen <ArrowRight className="ml-2" size={16} />
                    </span>
                    {project.githubLink && <Github size={18} className="text-gray-300 group-hover:text-black transition-colors" />}
                  </div>
                </div>
              </Link>
            </motion.div>
